...
<div class="postContainer">
    ...
    <?php foreach ($posts as $post) : ?>
     <div class="userpost" style="border: 0px" id="post<?php echo $post->id; ?>">
         ...
         <p><a href="<?php echo getUrl("/friends/profile.php?user={$post->username}") ?>" class="postUser" style="float: left;">
          ...
          <div class="userName"><?php echo $post->username; ?> </div></a>&nbsp; <i>shared a post.</i></p>
           </a>
                
                <?php
        echo'<p style="color:black;">'. $post->post."</p>";
    }  ?>
 ...
 <label style="color: green;" title="More options" options ><i class="fa fa-ellipsis-v fa-2x"></i></label>
                   <div style="position: relative;" id="show" class="profileEditModal hide" options-div><div style="position:absolute;background-color: whitesmoke;max-width: max-content;margin-left: -2.5cm;">
                     ...
                     <a href="<?php echo getUrl("/post/deletePost.php?id={$post->id}") ?>" onclick="return confirm('Are you sure you want to delete this post?')"
 delete-option>Delete Post <i class="fa fa-trash-o " style="color:red"></i></a></div> </div> </div>             <?php }  ?>
            </div>
            <div class="postBody">
                 <?php if ($post->post_id!=0) {
        $result=Post::findOne($post->post_id);
